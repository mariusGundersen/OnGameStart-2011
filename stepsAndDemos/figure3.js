Demo.event.onEnterFrame = function(t){
	...
	if(Demo.mouse.isDown){
		...
		l = Math.sqrt(norm.dx*norm.dx + norm.dy*norm.dy + norm.dz*norm.dz);
		norm.dx /= l;
		norm.dy /= l;
		norm.dz /= l;
		dp /= l;
		world.triangles[0].c = "rgba("+Math.round(Math.max(0, 255*dp))+", "+Math.round(Math.max(0, 204*dp))+", 0, 0.5)";
...
function render(world, camera){
	...
	camera.screen.beginPath();
	camera.screen.fillStyle = "#FC0";
	camera.screen.arc(world.vertices[7].posX, world.vertices[7].posY, world.vertices[7].posZ*50, 0, Math.PI*2, false);
	camera.screen.fill();
}
